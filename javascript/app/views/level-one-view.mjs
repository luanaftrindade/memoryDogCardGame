        ...
        if (!document.querySelectorAll('.card:not(.flipped)').length) {
            setTimeout(() => {
                ...
                internals.selectors.win.innerHTML = `
                    ...
                    <button class="levelTwo">Level Two</button>
...
internals.deleteAllHTML = function () {
    const gameContainer = internals.selectors.gameContainer;
    if (gameContainer) {
        gameContainer.innerHTML = '';
    } else {
        console.log("Could not find gameContainer to delete HTML.");
    }
}
...
externals.attachEventListeners = function (images) {
    document.addEventListener('click', event => {
        ...
        } else if (eventTarget.className.includes('play') && !eventTarget.className.includes('disabled')) {
            internals.selectors.play.remove();
            console.log(internals.selectors.gameContainer.innerHTML);
            internals.selectors.gameContainer.innerHTML = internals.createLevelOneHtml();
            internals.generateMemoryGame(images);
            console.log(internals.selectors.gameContainer.innerHTML);
        } else if (eventTarget.className.includes('start') && !eventTarget.className.includes('disabled')) {
            internals.startGame();
        } else if (eventTarget.className.includes('levelTwo') && !eventTarget.className.includes('disabled')) {
            internals.deleteAllHTML();
            levelTwoView.createLevelTwoHtml(images);
        }
    })
